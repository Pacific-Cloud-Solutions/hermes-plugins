...
from .schemas import NETWORK_EXPOSURE_SCHEMA, TOOL_NAME
...
def network_exposure(args, **kwargs) -> str:
    """Run the read-only local network exposure audit.
    ...
    try:
        ...
        report = core.Report(
            tool="pcs-security-network-exposure",
...
def register_all(ctx) -> None:  # noqa: ANN001 — PluginContext
    ...
    try:
        ...
        logger.warning(
            "pcs-security-network-exposure: pcs-security-core is unavailable (%s). %s will "
            "register but refuse to run until it is installed — no report is produced "
            "without the taint model.", exc, TOOL_NAME,
        )

    ctx.register_tool(
        name=TOOL_NAME,
        toolset=_TOOLSET,
        schema=NETWORK_EXPOSURE_SCHEMA,
        handler=network_exposure,
        emoji="🌐",
    )