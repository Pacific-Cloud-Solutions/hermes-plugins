...
from .schemas import LOG_TRIAGE_SCHEMA, TOOL_NAME
...
def log_triage(args, **kwargs) -> str:
    """Run the read-only local log triage.
    ...
    try:
        ...
        max_lines = _positive_int(args.get("max_lines"), 20000)
        failure_threshold = _positive_int(args.get("failure_threshold"), 5)

        audit = run_all(root, include_info, max_lines, failure_threshold)
        report = core.Report(
            tool="pcs-security-log-triage",
        ...
        return json.dumps({
            ...
            "verdict": "triage could not run — this is NOT a statement about the target",
        })
...
def register_all(ctx) -> None:  # noqa: ANN001 — PluginContext
    ...
    try:
        ...
        logger.warning(
            "pcs-security-log-triage: pcs-security-core is unavailable (%s). %s will register "
